...
projRoads_snap <- roads::projectRoadsNew(landings,costRaster,existingRoadsRast,roadMethod='snap')
...
projRoads_lcp <- roads::projectRoadsNew(landings, costRaster, existingRoadsRast, roadMethod = 'lcp')
...
projRoads_mst <- roads::projectRoadsNew(landings, costRaster, existingRoadsRast, roadMethod = 'mst')
...
text(land.pnts@coords,labels=land.pnts$set,cex=0.6,adj=c(0.5,0.3),xpd=TRUE)
```

### One-time simulation

If landings are specified to the `projectRoads` method as any valid object other than a multi-layer [`RasterStack`](https://www.rdocumentation.org/packages/raster/versions/2.9-5/topics/Raster-class) {[`raster`](https://cran.r-project.org/web/packages/raster/index.html)} or [`RasterBrick`](https://www.rdocumentation.org/packages/raster/versions/2.9-5/topics/Raster-class) {[`raster`](https://cran.r-project.org/web/packages/raster/index.html)} object, then a one-time road simulation will be performed.
...
oneTime_sim <- stack() ## empty raster stack will be updated with sim results rasters
for (i in 1:4){
  oneTime_sim <- stack(oneTime_sim,roads::projectRoads(land.pnts[land.pnts$set==i,],scen$cost.rast,
                                                       scen$cost.rast==0,roadMethod='mst')$roads)
}
...
par(mar=par('mar')/2)
for (i in 1:4){
  plot(scen$cost.rast,col=rastColours2,main=paste0('Landings set ',i),legend=FALSE)
  plot(oneTime_sim[[i]],col='grey30',add=TRUE,legend=FALSE)
  plot(land.pnts[land.pnts$set==i,],add=TRUE,pch=21,cex=1.5,bg='white')
}
...
independent <- any(!is.na(oneTime_sim))
...
Multi-temporal (multiple time steps) roads projections can be performed by the `projectRoads` method, by specifying landings in the form of a multi-layer [`RasterStack`](https://www.rdocumentation.org/packages/raster/versions/2.9-5/topics/Raster-class) {[`raster`](https://cran.r-project.org/web/packages/raster/index.html)} or [`RasterBrick`](https://www.rdocumentation.org/packages/raster/versions/2.9-5/topics/Raster-class) {[`raster`](https://cran.r-project.org/web/packages/raster/index.html)} object.
...
multiTime_sim <- roads::projectRoads(land.stack,scen$cost.rast,
                                     scen$cost.rast==0,roadMethod='mst')
multiTime_sim[multiTime_sim==0] <- NA ## set non-road to NA for display purposes
## rather than a list, projectRoads returns a RasterBrick with simulated roads for each time point
par(mfrow=c(3,2))
par(mar=par('mar')/2)
for (i in 1:nlayers(multiTime_sim)){
  plot(scen$cost.rast,col=rastColours2,main=paste0('Roads at time t=',(i-1)),legend=FALSE)
  plot(multiTime_sim[[i]],col='grey30',add=TRUE,legend=FALSE)
  if (i >= 2){
    plot(land.pnts[land.pnts$set<(i-1),],add=TRUE,pch=1,cex=1.5)
    plot(land.pnts[land.pnts$set==(i-1),],add=TRUE,pch=21,cex=1.5,bg='white')
  }
}
```


If preparing the your landings as a [`RasterStack`](https://www.rdocumentation.org/packages/raster/versions/2.9-5/topics/Raster-class) {[`raster`](https://cran.r-project.org/web/packages/raster/index.html)} or [`RasterBrick`](https://www.rdocumentation.org/packages/raster/versions/2.9-5/topics/Raster-class) {[`raster`](https://cran.r-project.org/web/packages/raster/index.html)} object is an issue, the same results can also be acheived by chaining calls to the `projectRoads` method, but altering the state of existing roads, as well as the corresponding cost raster, for each call.

```{r, fig.show='hold'}
costRast <- scen$cost.rast
existing_roads <- costRast==0
multiTime_sim2 <- stack(existing_roads) ## first element in output stack is roads at time t=0
for (i in 1:4){
  newroads <- roads::projectRoads(land.pnts[land.pnts$set==i,],costRast,
                                  existing_roads,roadMethod='mst')$roads
  existing_roads <- newroads > 0  ## new roads become existing roads for next time step
  costRast[existing_roads] <- 0   ## update costRast under existing roads (value of 0)
  multiTime_sim2 <-stack(multiTime_sim2,existing_roads) ## update stack
}
multiTime_sim2[multiTime_sim2==0] <- NA ## set non-road to NA for display purposes
## plot
par(mfrow=c(3,2))
par(mar=par('mar')/2)
for (i in 1:nlayers(multiTime_sim2)){
  plot(scen$cost.rast,col=rastColours2,main=paste0('Roads at time t=',(i-1)),legend=FALSE)
  plot(multiTime_sim2[[i]],col='grey30',add=TRUE,legend=FALSE)
  if (i >= 2){
    plot(land.pnts[land.pnts$set<(i-1),],add=TRUE,pch=1,cex=1.5)
    plot(land.pnts[land.pnts$set==(i-1),],add=TRUE,pch=21,cex=1.5,bg='white')
  }
