...
#'
#'@param costRast A RasterLayer represening the cost of new roads. Existing road
#'  network will be identiifed by cells with a cost of zero.
#'@param landings SpatialPointsDataFrame*, SpatialPoints*,
#'  SpatialPolygonsDataFrame*, SpatialPolygons*, RasterLayer, RasterStack, or
#'  RasterBrick. Represents landing locations, or areas in the case of polygons.
#'  Layers in RasterStack or RasterBrick objects are assumed to represent
#'  multiple time points. If NA or NULL, landing locations will not be inculuded
#'  in the visualization.
#'@param projRoadsResults NA, NULL, or one of the following return types from
#'  projectRoads:  list or RasterBrick. If NA or NULL, projected roads will not
#'  be inculuded in the visualization.
#'@param col.cost A vector of colours (as returned by a colour palette).
#'  Represents colours for displaying cost. If NA or NULL, a default colour
#'  palette will be used, based on colorRamps::matlab.like.
#'@param main Character. The main title for the visualization.
#'@param xlim An integer vector of length 2 representing the minumum[0] and
#'  maximum[1] of the range (in the x direction) of the cost raster that is to
#'  be shown. If NA or NULL, the full range of the cost raster will be shown in
#'  the x direction.
#'@param ylim An integer vector of length 2 representing the minumum[0] and
#'  maximum[1] of the range (in the y direction) of the cost raster that is to
#'  be shown. If NA or NULL, the full range of the cost raster will be shown in
#'  the y direction.
#'@param height Numeric representing the height, in centimeters of the graphics
#'  device (window) containing the output visualization. Default is 15.
#'@param out.file A character representing the path to an output image file.  If
#'  NA or NULL, the visualization will be generated in a new graphics device. If
#'  a path to a file is provided, an image file will be generated at the
#'  specified location, containing the visualization. Supported file types are
#'  .png, .jpg, .bmp, .tif, .pdf.
#' @examples
...
#'             main = 'Scenario 1',
#'             height = 20,
#'             out.file = 'myVisualization.png')
#' }
#' 
#' # demonstration scenario 2
#' scen <- demoScen[[2]] 
#' # use landing polygons
#' landings <- scen$landings.poly 
...
#' scen <- demoScen[[3]] 
#' # visualize just the cost raster and existing roads network
#' visualize(scen$cost.rast)
#' # use landing sets 1 to 4 of the landings RasterStack for multi-temporal roads
#' # projection
...
#'                    roadMethod = 'mst')
#' # visualize results
#' visualize(scen$cost.rast, landings, pr) 
