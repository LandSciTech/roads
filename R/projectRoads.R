...
#'    }else{
#'       lapply(dim(projRoadsResults)[3]:2,function(i){
#'          newRoads <- projRoadsResults[[i]]
#'          newRoads[!newRoads] <- NA
#'          plot(newRoads,col=grey.colors(dim(projRoadsResults)[3]-1)[i],add=T,legend=F)
#'       })
#'       points(landings,pch=21,cex=2,bg="white")
#'    }
#' }
#'
#' # project roads: using scnario 1 / landings as a SpatialPointsDataFrame / the least-cost path ("lcp") approach 
...
#' landings.points <- scen$landings.points[scen$landings.points$set==5,] # use landing set 5 of of demo scenario 2 
...
#' landings.points <- scen$landings.points[scen$landings.points$set%in%1:4,] # use landing sets 1 to 4 of of demo scenario 3 
...
#' visualize(projRoadsResults,landings.points,scen)
#' ### NOT COMPLETE.. NEED TO FIX PLOTS FOR STACK OUTPUT
#' 
#' @export
